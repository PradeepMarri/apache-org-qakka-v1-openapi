...
func AckmessageHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/queues/%s/messages/%s", cfg.BaseURL, queueName, queueMessageId)
...
func CreateAckmessageTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_queues_queueName_messages_queueMessageId",
		mcp.WithDescription("Acknowledge that Queue Message has been processed."),
		mcp.WithString("queueName", mcp.Required(), mcp.Description("Name of Queue")),
		mcp.WithString("queueMessageId", mcp.Required(), mcp.Description("ID of Queue Message to be acknowledged")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    AckmessageHandler(cfg),
	}
}
