...
func GetqueueconfigHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetqueueconfigTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_queues_queueName_config",
		mcp.WithDescription("Get Queue config."),
		mcp.WithString("queueName", mcp.Required(), mcp.Description("Name of Queue")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetqueueconfigHandler(cfg),
	}
}
